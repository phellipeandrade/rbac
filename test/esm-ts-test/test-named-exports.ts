...
class MockRoleAdapter {
  ...
  async addRole(roleName: string, role: any, tenantId?: string): Promise<void> {
  ...
  async updateRoles(roles: any, tenantId?: string): Promise<void> {
    const tid = tenantId || 'default';
...
async function testCreateTenantRBAC() {
  ...
  const canFind: boolean = await rbacInstance.can('user', 'products:find');
  const canDelete: boolean = await rbacInstance.can('user', 'products:delete');
  const adminCanDelete: boolean = await rbacInstance.can('admin', 'products:delete');
...
async function testTenantIsolation() {
  ...
  const tenantACanFindProducts: boolean = await tenantA.can('user', 'products:find');
  ...
  const tenantBCanFindProducts: boolean = await tenantB.can('user', 'products:find');
  ...
  const tenantBCanFindOrders: boolean = await tenantB.can('user', 'orders:find');
...
const defaultExport: typeof RBAC = RBAC;
const namedExport: typeof createTenantRBAC = createTenantRBAC;
