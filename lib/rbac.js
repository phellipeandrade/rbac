...
var logPermission = function logPermission(role, operation, result) {
  var fResult = result ? "\x1B[1;32m".concat(result, "\x1B[1;34m") : "\x1B[1;31m".concat(result, "\x1B[1;34m");
  var fRole = "\x1B[1;32m".concat(role, "\x1B[1;34m");
  var fOperation = "\x1B[1;32m".concat(operation, "\x1B[1;34m");

  try {
    console.log('\x1b[33m%s\x1b[0m: ', underline()); // yellow

    console.log('\x1b[1;34m%s\x1b[0m: ', "RBAC - ROLE: [".concat(fRole, "] OPERATION: [").concat(fOperation, "] PERMISSION: [").concat(fResult, "]"));
    console.log('\x1b[33m%s\x1b[0m: ', underline());
  } catch (e) {}
};

var can = function can(mappedRoles) {
  return function (role, operation, params) {
    return new Promise(function (resolve, reject) {
      ...
      var rejectPromise = function rejectPromise(role, result) {
        logPermission(role, operation, result);
      ...
      if (!FoundedRole.can.includes(operation)) {
        ...
        return Promise.all(FoundedRole.inherits.map(function (parent) {
          return can(parent, operation, params);
...
var mapRoles = function mapRoles(roles) {
  if ((0, _typeof2.default)(roles) !== 'object') {
